    ...
    <div class="row mt-4">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <!-- Header -->
                    <h4 class="card-title text-center text-3xl font-bold">Update Product Subcategory</h4>
                    @if (session('success'))
                    ...
                    <form class="form-horizontal p-t-20" action="{{ route('subcategory.update', $subcategory->id) }}"
                        ...
                        <div class="form-group row mb-4">
                            ...
                            <div class="col-sm-9">
                                <textarea
                                    class="form-control bg-slate-700 border border-slate-600 rounded-lg text-slate-200 focus:ring-blue-500 focus:border-blue-500"
                                    id="description" name="description" rows="4" placeholder="Enter subcategory description">{{ old('description', trim($subcategory->description) === 'Not given.' ? '' : trim($category->description)) }}</textarea>
                                @error('description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
